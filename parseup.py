...
#for location_arrays in survey_data['location'][0]:
#    for location_dictionary in location_arrays:
#        print location_dictionary

loc_0 = [x for x in survey_data['location']][0]

loc_1 = [x for x in survey_data['location']][1]
loc_2 = [x for x in survey_data['location']][2]

# find n-1, and n+1 slopes, average together

# change in distance == euclidean distance

# distance=sqrt((lat1 + lat2)**2 + (lon1 + lon2)**2)
# array of time is UnixTimestamp
array_of_longitudes = []
array_of_latitudes = []
array_of_time = []
...
for x in loc_2:
    ...
    array_of_time.append(x[u'TimeCompleted'])
...
def euclid_slope_lat_long(latitude_array, longitude_array, time_list):
    ...
    while index_of_n + 1< len(latitude_array):
        ...
        time_difference_a = time_difference_a / (3600.00 * 1000)
        velocities.append(distance / time_difference_a)
        times.append(int(str(time_list[index_of_n])[0:-3]))
        index_of_n +=1
    return times, velocities

import collections

data =  euclid_slope_lat_long(array_of_latitudes, array_of_longitudes, array_of_time)
import time
...
p = figure(title="Velocity Vs Time Using Raw GPS Data", x_axis_label='Date Time', y_axis_label='KPH', )
